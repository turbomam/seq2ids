...
# may want to use data/sqlite_not_postgres.db instead

.PHONY: uniprot_approach load_seqs_blast_result clean blast_res_to_sqlite  nt_approach all sqlite_input
...
uniprot_sqlite_input: clean target/seq2ids_v_uniprot.tsv blast_res_to_sqlite
...
target/seq2ids.fasta:
	poetry run seq2ids \
		--sqlite_file data/sqlite_not_postgres.db \
		--fasta_out $@ \
